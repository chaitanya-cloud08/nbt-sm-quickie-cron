...
// etc). Rather than hard-coding one shape, we walk the parsed JSON looking for
// objects that look like an article (a headline-ish string field, ideally with
// a summary-ish field alongside it) and normalize whatever we find.
...
function collectArticles(node, out, seen) {
  ...
  if (title && !seen.has(title)) {
    seen.add(title);
    out.push({ title, synopsis: firstString(node, SUMMARY_KEYS) || "" });
  }
